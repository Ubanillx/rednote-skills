...
def collect_note(note_url: str, delay_seconds: float = 0.0) -> str:
    """
    收藏小红书笔记
    ...
    try:
        ...
        if state.get("collected"):
            return "📥 当前笔记已收藏，跳过重复操作"

        click_interact_action(
            page,
            action_name="收藏笔记",
            button_selector=COLLECT_BUTTON_SELECTOR,
            state_key="collected",
            expected_state=True,
...
if __name__ == "__main__":
    ...
    result = collect_note(note_url, delay_seconds)
    print(result)
